...
window.goToStep = function goToStep(step) {
  if (step === 2) {
    ...
    if (!pass || pass.length < 8) {
      ...
      window.showToast('A palavra-passe deve ter no mínimo 8 caracteres.');
      return;
    } else if (passInput) {
  ...
  row.innerHTML = `
    ...
    <div class="funcionario-row-bottom">
      <div class="field-with-action">
        <input class="field-input" id="fn-pass-${idx}" name="funcionarios[${idx}][password]" type="password" minlength="8" placeholder="Senha (Mín 8 chars)" style="padding-right:104px;" required>
