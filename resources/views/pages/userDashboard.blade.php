...
<style>
  /* ===== TOKENS ===== */
  :root{
    --bg-deep:#0B0B0D; --bg-panel:#141416; --bg-elevated:#1A1A1D;
    --field-bg:#1F1F22; --field-border:#333336; --line:#2A2A2D;
    --gold:#D4AF37; --gold-soft:rgba(212,175,55,0.12); --gold-dark:#241C05;
    --text-primary:#F5F3EE; --text-secondary:#8F8D87; --text-faint:#5C5A54;
    --ok:#4CAF6D; --ok-soft:rgba(76,175,109,0.12);
    --warn:#E0A93B; --warn-soft:rgba(224,169,59,0.12);
    --danger:#D64545; --danger-soft:rgba(214,69,69,0.12);
    --info:#4C93AF; --info-soft:rgba(76,147,175,0.12);
    --radius:14px; --topbar-h:64px; --nav-h:68px;
  }
  *{ box-sizing:border-box; margin:0; padding:0; -webkit-tap-highlight-color:transparent; }
  button:focus-visible,a:focus-visible,input:focus-visible{ outline:2px solid var(--gold); outline-offset:2px; }
  html,body{ min-height:100%; font-family:'Inter',sans-serif; background:#050506; color:var(--text-primary); overflow-x:hidden; max-width:100vw; }
  ::-webkit-scrollbar{ width:6px; height:6px; }
  ::-webkit-scrollbar-thumb{ background:var(--field-border); border-radius:8px; }
  button{ font-family:'Inter',sans-serif; }

  .app-shell{ max-width:460px; min-height:100vh; min-height:100dvh; margin:0 auto; background:var(--bg-deep); position:relative; display:flex; flex-direction:column; overflow-x:hidden; box-shadow:0 0 60px rgba(0,0,0,0.6); }
  @media(min-width:640px){ body{ background:radial-gradient(circle at 50% 0%,#16161a 0%,#050506 65%); padding:28px 0; } .app-shell{ min-height:calc(100dvh - 56px); border-radius:28px; overflow:hidden; border:1px solid var(--line); } }

  /* TOPBAR */
  .topbar{ position:sticky; top:0; z-index:40; height:var(--topbar-h); display:flex; align-items:center; justify-content:space-between; gap:10px; padding:0 18px; background:rgba(20,20,22,0.9); backdrop-filter:blur(10px); border-bottom:1px solid var(--line); flex-shrink:0; }
  .topbar-brand{ display:flex; align-items:center; gap:10px; min-width:0; }
  .brand-mark{ width:34px; height:34px; border-radius:9px; flex-shrink:0; background:var(--gold); color:var(--gold-dark); display:flex; align-items:center; justify-content:center; font-family:'Oswald',sans-serif; font-weight:700; font-size:14px; overflow:hidden; }
  ...
  .user-avatar{ width:34px; height:34px; border-radius:50%; background:linear-gradient(135deg,var(--gold),#8a6d1f); display:flex; align-items:center; justify-content:center; color:var(--gold-dark); font-weight:700; font-size:12.5px; flex-shrink:0; cursor:pointer; border:2px solid var(--bg-panel); }
  .shift-dot{ width:8px; height:8px; border-radius:50%; background:var(--ok); box-shadow:0 0 0 3px var(--ok-soft); }

  /* CONTENT */
  .content{ flex:1; padding:18px 18px calc(var(--nav-h) + 26px); width:100%; overflow-x:hidden; }
  ...
  .step-seg.done .step-seg-fill,.step-seg.current .step-seg-fill{ width:100%; }
  ...
  .step-back{ display:inline-flex; align-items:center; gap:6px; background:none; border:none; color:var(--text-secondary); font-size:12.5px; font-weight:500; cursor:pointer; margin-bottom:14px; padding:4px 0; }
  .step-back:hover{ color:var(--gold); }

  /* SERVICE GRID */
  .service-grid{ display:grid; grid-template-columns:1fr 1fr; gap:12px; }
  .service-card{ background:var(--bg-panel); border:1.5px solid var(--line); border-radius:var(--radius); padding:18px 14px; display:flex; flex-direction:column; gap:10px; cursor:pointer; transition:border-color .15s ease,background .15s ease,transform .1s ease; min-height:100px; }
  .service-card:active{ transform:scale(0.97); }
  .service-card:hover{ border-color:var(--gold); }
  .service-card.selected{ border-color:var(--gold); background:var(--gold-soft); }
  .service-icon{ width:34px; height:34px; border-radius:9px; background:var(--field-bg); display:flex; align-items:center; justify-content:center; color:var(--gold); }
  ...
  .payment-card{ display:flex; align-items:center; gap:14px; background:var(--bg-panel); border:1.5px solid var(--line); border-radius:var(--radius); padding:16px; cursor:pointer; transition:border-color .15s ease,background .15s ease,transform .1s ease; }
  ...
  .payment-icon{ width:42px; height:42px; border-radius:11px; flex-shrink:0; display:flex; align-items:center; justify-content:center; }
  ...
  /* CONFIRM / SUMMARY */
  .summary-card{ background:var(--bg-panel); border:1px solid var(--line); border-radius:var(--radius); padding:22px 20px; margin-bottom:18px; }
  ...
  /* BUTTONS */
  .btn{ display:flex; align-items:center; justify-content:center; gap:9px; width:100%; font-family:'Inter',sans-serif; font-weight:700; font-size:15px; padding:16px; border-radius:12px; border:none; cursor:pointer; transition:filter .15s ease,transform .1s ease; }
  ...
  /* SUCCESS OVERLAY */
  .success-overlay{ position:fixed; inset:0; z-index:200; background:var(--bg-deep); display:none; align-items:center; justify-content:center; flex-direction:column; gap:16px; text-align:center; padding:30px; }
  .success-overlay.show{ display:flex; }
  .success-ring{ width:88px; height:88px; border-radius:50%; background:var(--ok-soft); display:flex; align-items:center; justify-content:center; color:var(--ok); animation:pop .35s ease; }
  ...
  .history-item{ display:flex; align-items:center; gap:12px; background:var(--bg-panel); border:1px solid var(--line); border-radius:12px; padding:13px 14px; margin-bottom:10px; }
  .history-time{ width:44px; text-align:center; flex-shrink:0; font-family:'Oswald',sans-serif; font-weight:600; font-size:13px; color:var(--gold); }
  ...
  .pay-badge{ font-size:10px; font-weight:600; padding:3px 8px; border-radius:20px; display:inline-flex; align-items:center; gap:4px; white-space:nowrap; }
  ...
  .profile-avatar{ width:76px; height:76px; border-radius:50%; background:linear-gradient(135deg,var(--gold),#8a6d1f); display:flex; align-items:center; justify-content:center; color:var(--gold-dark); font-family:'Oswald',sans-serif; font-weight:700; font-size:24px; margin-bottom:12px; }
  ...
  .profile-salon-chip{ display:inline-flex; align-items:center; gap:6px; margin-top:10px; background:var(--gold-soft); color:var(--gold); font-size:11.5px; font-weight:600; padding:5px 12px; border-radius:20px; }
  ...
  .list-link-row{ display:flex; align-items:center; gap:12px; padding:15px 0; border-top:1px solid var(--line); color:var(--text-primary); cursor:pointer; }
  ...
  /* MODAL */
  .modal-overlay{ position:fixed; inset:0; background:rgba(0,0,0,0.65); display:none; align-items:flex-end; justify-content:center; z-index:150; }
  .modal-overlay.open{ display:flex; }
  .modal{ background:var(--bg-panel); border:1px solid var(--line); border-top-left-radius:20px; border-top-right-radius:20px; width:100%; max-width:460px; padding:22px 22px calc(22px + env(safe-area-inset-bottom)); animation:slideUp .2s ease; }
  @keyframes slideUp{ from{ transform:translateY(30px); opacity:0; } to{ transform:none; opacity:1; } }
  .modal-grip{ width:36px; height:4px; border-radius:4px; background:var(--field-border); margin:0 auto 18px; }
  .modal-icon{ width:46px; height:46px; border-radius:12px; background:var(--danger-soft); color:var(--danger); display:flex; align-items:center; justify-content:center; margin-bottom:14px; }
  ...
  /* TOAST */
  .toast{ position:fixed; bottom:calc(var(--nav-h) + 14px); left:50%; transform:translateX(-50%) translateY(20px); background:var(--bg-elevated); border:1px solid var(--gold); color:var(--text-primary); padding:0; border-radius:12px; font-size:13px; opacity:0; pointer-events:none; transition:opacity .25s ease,transform .25s ease; z-index:200; box-shadow:0 10px 30px rgba(0,0,0,0.5); max-width:88vw; overflow:hidden; }
  ...
  .toast-undo{ flex-shrink:0; background:none; border:none; color:var(--gold); font-weight:700; font-size:12.5px; padding:8px 14px; cursor:pointer; white-space:nowrap; border-left:1px solid var(--field-border); margin-left:2px; display:none; }
  ...
  /* BOTTOM NAV */
  .bottom-nav{ position:sticky; bottom:0; left:0; right:0; z-index:40; background:rgba(20,20,22,0.95); backdrop-filter:blur(10px); border-top:1px solid var(--line); padding:9px 8px calc(9px + env(safe-area-inset-bottom)); display:flex; justify-content:space-around; flex-shrink:0; }
  .nav-tab{ display:flex; flex-direction:column; align-items:center; gap:4px; color:var(--text-faint); font-size:10.5px; font-weight:600; padding:6px 18px; border-radius:10px; cursor:pointer; transition:color .15s ease; position:relative; }
  ...
  <div class="app-shell">
    ...
    <main class="content">
      ...
      <section class="view active" id="view-registar">
        ...
        <div class="greeting">
          <div class="greeting-title">Olá, {{ $user->name }} 👋</div>
      ...
      <section class="view" id="view-perfil">
        ...
        <div class="profile-header">
          <div class="profile-avatar">{{ strtoupper(collect(explode(' ', $user->name))->take(2)->map(fn($w) => $w[0])->join('')) }}</div>
          ...
          <div class="profile-salon-chip">
            ...
            <span>{{ $user->barbearia->name ?? 'Sem salão' }}</span>
          </div>
        </div>
  ...
  function confirmarAtendimento(){
    ...
    .then(() => {
      setTimeout(() => {
        ...
        // Atualiza os KPIs do "Meu Dia" sem recarregar a página
